...
from Ankit.__main__ import STATS, TOKEN, USER_INFO
import Ankit.modules.sql.userinfo_sql as sql
from Ankit.modules.disable import DisableAbleCommandHandler
from Ankit.modules.sql.global_bans_sql import is_user_gbanned
from Ankit.modules.sql.afk_sql import is_afk, check_afk_status
from Ankit.modules.sql.users_sql import get_user_num_chats
from Ankit.modules.helper_funcs.chat_status import sudo_plus
from Ankit.modules.helper_funcs.extraction import extract_user
from Ankit import telethn as FallenTelethonClient, TIGERS, DRAGONS, DEMONS
...
def info(update: Update, context: CallbackContext):
    ...
    if user_id not in [bot.id, 2127221861]:
    ...
    if disaster_level_present:
        text += ' \n[<a href="[ᴀɴᴋɪᴛ 🇮🇳](t.me/XnKiTKuMaR]">ᴄʟɪᴄᴋ ʜᴇʀᴇ ᴛᴏ ᴋɴᴏᴡ ᴡʜᴀᴛ ɪs ᴅɪsᴀsᴛᴇʀ ʟᴇᴠᴇʟs.</a>]'.format(
            bot.username
        )
...
def set_about_me(update: Update, context: CallbackContext):
    ...
    if message.reply_to_message:
        ...
        if repl_user_id in [bot.id, 2127221861] and (user_id in DEV_USERS):
...
def set_about_bio(update: Update, context: CallbackContext):
    ...
    if message.reply_to_message:
        ...
        if user_id in [2127221861] and sender_id not in DEV_USERS:
